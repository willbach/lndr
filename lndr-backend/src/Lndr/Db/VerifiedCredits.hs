...
lookupCreditByAddress addr conn = query conn "SELECT ucac, creditor, debtor, verified_credits.amount, nonce, memo FROM verified_credits LEFT JOIN settlements USING(hash WHERE (creditor = ? OR debtor = ?) AND settlements.verified IS DISTINCT FROM FALSE ORDER BY verified_credits.created_at DESC" (addr, addr)
...
lookupSettlementCreditByHash :: Text -> Connection -> IO (Maybe BilateralCreditRecord)
lookupSettlementCreditByHash hash conn = listToMaybe <$> query conn sql (Only hash)
